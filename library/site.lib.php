    ...
    class Site {
		...
		public function GetPage() {
			...
			$result = strtr($krnModule->GetResult(), array(
		    	...
		    	'&lt;%CITYGEN%&gt;'			=> $_SESSION['ClientUser']['City']['TitleGen'],
		    	'&lt;%CITYGEN2%&gt;'		=> $_SESSION['ClientUser']['City']['TitleGen2'],
		    	'<%AVSUM'.SRO_BUILDERS_TYPE_ID.'%>'				=> number_format($avsum[SRO_BUILDERS_TYPE_ID], 0, '', '&nbsp;'),
		    	'&lt;%AVSUM'.SRO_BUILDERS_TYPE_ID.'%&gt;'		=> number_format($avsum[SRO_BUILDERS_TYPE_ID], 0, '', '&nbsp;'),
		    	'<%AVSUM'.SRO_PROJECTERS_TYPE_ID.'%>'			=> number_format($avsum[SRO_PROJECTERS_TYPE_ID], 0, '', '&nbsp;'),
		    	'&lt;%AVSUM'.SRO_PROJECTERS_TYPE_ID.'%&gt;'		=> number_format($avsum[SRO_PROJECTERS_TYPE_ID], 0, '', '&nbsp;'),
		    	'<%AVSUM'.SRO_PROSPECTORS_TYPE_ID.'%>'			=> number_format($avsum[SRO_PROSPECTORS_TYPE_ID], 0, '', '&nbsp;'),
		    	'&lt;%AVSUM'.SRO_PROSPECTORS_TYPE_ID.'%&gt;'	=> number_format($avsum[SRO_PROSPECTORS_TYPE_ID], 0, '', '&nbsp;'),
		    	'<%BREAD_CRUMBS%>'			=> '',
