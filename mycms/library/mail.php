...
class Mail {	
  ...
  /* private functions used internally */
	...
	public function SendMailTemplateFromSite($to,$template_code,$params=array(),$attachments=array()){
		$template=dbGetRecordFromDb('SELECT * FROM email_templates WHERE `Code`="'.$template_code.'"',__FILE__,__LINE__);
		$_params=array();
		foreach($params as $key=>$val){
			$_params['%'.strtolower($key).'%']=$val;
		}
		$subject=strtr($template['Subject'],$_params);
		//$text=strtr(nl2br($template['Text']),$_params);
		$text=strtr($template['Text'],$_params);
		return $this->SendMailFromSite($to,$subject,$text,$attachments);
	}
